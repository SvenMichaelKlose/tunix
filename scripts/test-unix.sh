...
tests () {
    ...
    for i in NO_BUILTIN_APPEND NO_BUILTIN_CHAR_AT NO_BUILTIN_NCONC NO_BUILTIN_NTHCDR NO_BUILTIN_SUBSEQ; do
        $1 "$PARANOID -D$i"
    done
    for i in NO_BUILTIN_GROUP_BITOPS NO_BUILTIN_GROUP_DIRECTORY NO_BUILTIN_GROUP_IMAGE NO_BUILTIN_GROUP_RAW_ACCESS NO_BUILTIN_GROUP_SYMBOL_NAME; do
        $1 "$PARANOID -D$i"
    done
    $1 "-DNAIVE -DEXIT_FAILURE_ON_ERROR -DLOAD_ALL $VERBOSE"
}
